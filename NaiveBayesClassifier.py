...
def classifier(train_dataset1, train_dataset2, testRow, my, sy, mn, sn):
    ...
    for category in numerical:
        prob_yes = calculateProbability(
            testRow[numericalIndexes[i]], means_yes[category], stds_yes[category])
        c_yes += math.log(prob_yes, 10)
        prob_no = calculateProbability(
            testRow[numericalIndexes[i]], means_no[category], stds_no[category])
...
def train_and_test():
    ...
    expected=test_dataset['RainTomorrow'].values.tolist()
    ...
    for row in test_dataset.values.tolist():
        prediction = classifier(train_dataset_yes, train_dataset_no,
              row, means_yes, stds_yes, means_no, stds_no)
        print(prediction)
        # if prediction == 1 and expected[total] == ' Yes':
        #     right +=1
        # if prediction == 0 and expected[total] == ' No':
        #     right +=1
        # total += 1
    
    end = time.time()
    # print('Results for running naive bayes classifier:')
